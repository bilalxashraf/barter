import { getPaymentLinkDetails } from '../../_lib/agentApi';
import PaymentCheckout from './PaymentCheckout';
import BackButton from '../../components/BackButton';

export const metadata = {
  title: 'Payment Checkout — Barter Payments'
};

export default async function PaymentPage({ params }: { params: Promise<{ linkId: string }> }) {
  const { linkId } = await params;

  let paymentData;
  let error: string | null = null;

  try {
    paymentData = await getPaymentLinkDetails(linkId);
  } catch (err: any) {
    error = err.message || 'Failed to load payment link';
  }

  return (
    <main className="dashboard">
      <section>
        <div className="container">
          <header className="nav">
            <div className="nav-left">
              <BackButton />
              <a className="nav-logo" href="/">
                <img src="/BarterPaymentLogo.png" alt="Barter Payments logo" width={36} height={36} />
                <span>Barter Payments</span>
              </a>
            </div>
            <nav className="nav-links">
              <a href="/docs">Docs</a>
              <a href="/dashboard">Dashboard</a>
            </nav>
            <div className="nav-actions">
              <a className="nav-social" href="https://x.com/barterpayments" target="_blank" rel="noreferrer" aria-label="Barter Payments on X">
                <svg viewBox="0 0 24 24" aria-hidden="true">
                  <path d="M18.244 2.25h3.308l-7.227 8.26 8.507 11.24H16.32l-5.11-6.675-5.84 6.675H2.06l7.73-8.84L1.61 2.25h6.676l4.62 6.11 5.338-6.11Zm-1.162 17.52h1.833L6.68 4.126H4.72l12.362 15.645Z" />
                </svg>
                <span>@barterpayments</span>
              </a>
            </div>
          </header>
        </div>
      </section>

      <section>
        <div className="container">
          <div className="page-hero">
            <h1>Payment Checkout</h1>
            <p>Complete your payment securely with crypto</p>
          </div>

          {error ? (
            <div className="card" style={{ maxWidth: 600, margin: '0 auto', borderColor: 'rgba(255, 0, 0, 0.3)' }}>
              <h4>Error</h4>
              <p>{error}</p>
              <a href="/" className="button secondary" style={{ marginTop: 16 }}>
                Go Home
              </a>
            </div>
          ) : paymentData ? (
            <PaymentCheckout
              link={paymentData.link}
              recipientAddress={paymentData.recipientAddress}
              tokenAddress={paymentData.tokenAddress}
              decimals={paymentData.decimals}
            />
          ) : (
            <div className="card" style={{ maxWidth: 600, margin: '0 auto' }}>
              <p>Loading payment details...</p>
            </div>
          )}
        </div>
      </section>

      <footer className="footer">
        <div className="container footer-inner">
          <strong>Barter Payments</strong>
          <nav className="footer-links">
            <a href="/docs">Docs</a>
            <a href="/about">About</a>
            <a href="/contact">Contact</a>
            <a href="/legal/privacy">Privacy</a>
            <a href="/legal/terms">Terms</a>
          </nav>
        </div>
      </footer>
    </main>
  );
}
